...
from model import (
    cargo_prefetch_loop,
    generate_cargo_batch,
    generate_random_cargo,
    get_cargo_buffer_size,
    prefill_cargo_buffer,
    preview_cargo_batch,
)
...
def make_config(script_path):
    ...
    return {
        ...
        "map_file": output_dir / "shipments_map_09.html",
        "map_data_file": output_dir / "shipments_data_09.json",
        "map_template_file": base_dir / "shipments_map_template_09.html",
        "map_url_path": (output_dir / "shipments_map_09.html").relative_to(base_dir).as_posix(),
...
def make_package_factory(fake_service, rng, country_pool, timestamp_supplier):
    ...
    def create_package():
        ...
        cargo = generate_random_cargo(rng)
...
def run(settings=None):
    ...
    cargo_thread, generator_thread, monitor_thread, map_thread, manager_thread = start_background_threads(
        runtime,
        config,
        settings["delay"],
        create_package,
